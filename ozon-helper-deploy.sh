...
DEPLOY_DIST="/var/www/ozon-helper/miniapp"
...
echo "[BACKEND] Update docker-compose.yml"
sed -i "s|image: ${IMAGE_NAME}:.*|image: ${BACKEND_TAG}|g" "$COMPOSE_FILE"

echo "[BACKEND] Restart container..."
docker-compose -f "$COMPOSE_FILE" stop backend
docker-compose -f "$COMPOSE_FILE" up -d backend

# Frontend
cd "$FRONTEND_DIR"
echo "[FRONTEND] Build image for static export..."
docker build -t ozon-helper-frontend-export .

echo "[FRONTEND] Extracting dist/ → $DEPLOY_DIST"
TEMP_CONTAINER="temp-frontend-${DATE_TAG}"
docker create --name "$TEMP_CONTAINER" ozon-helper-frontend-export
rm -rf "$DEPLOY_DIST"
mkdir -p "$DEPLOY_DIST"
docker cp "$TEMP_CONTAINER":/export "$DEPLOY_DIST"
docker rm "$TEMP_CONTAINER"

# Очистка образов
if [ "$CLEAN_IMAGES" = "yes" ]; then
  echo "[CLEANUP] Removing old backend images..."
  ...
  docker images "ozon-helper-frontend-export" --format "{{.Repository}}:{{.Tag}}" | sort -r | tail -n +6 | xargs -r docker rmi
fi
